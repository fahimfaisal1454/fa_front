...
const AddOwePage = () => {
    ...
    const oweId = searchParams.get("id"); // for edit mode
    ...
    // ✅ Fetch borrower details if editing
    useEffect(() => {
        if (oweId) {
            const fetchBorrower = async () => {
                try {
                    const res = await AxiosInstance.get(`owe/${oweId}/`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (oweId) {
                await AxiosInstance.put(`owe/${oweId}/`, formData);
            } else {
                await AxiosInstance.post("owe/", formData);
            }
            router.push("/owe/oweList");
        } catch (error) {
            console.error("Error submitting owe:", error);
        }
    };
        ...
        <div className="max-w-6xl mx-auto p-6">
            ...
            >
                <div className="col-span-4 sm:col-span-2">
                    <label className="block text-gray-700 font-medium">Owe Name</label>
                    <input
                        type="text"
                        name="owed_name"
                        value={formData.owed_name}
                        ...
                        placeholder="Enter owe name"
                    />
                </div>

                <div className="col-span-4 sm:col-span-2">
                    <label className="block text-gray-700 font-medium">Owe Type</label>
                    <select
                        name="owed_type"
                        value={formData.owed_type}
                        onChange={handleChange}
                        className="w-full px-4 py-1 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition"
                    >
